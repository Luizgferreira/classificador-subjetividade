...
from code.data.getData import getCorpus, getEmbeddings
from code.models import correlation
from code.manage.manage import create_model, train
...
def CV(options, main_path,k=10, correlated=None):
    ...
    for i in range(k):
        ...
        train(options, cross_validation=True, corpus = X_train, labels=y_train)
        clf, vectorizer = create_model(options, main_path)
        X_test = [vectorizer.transform([sentence]) for sentence in X_test]
        results = [clf.predict(sentence) for sentence in X_test]
